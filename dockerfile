...
RUN mkdir /webapp
COPY . /webapp
CMD cd webapps;mvn clean install
CMD /webapp/target/*.war /usr/share/tomcat/webapps
CMD /opt/apache-tomcat-7.0.61/bin/startup.sh && tail -f /usr/share/tomcat/logs/catalina.out
